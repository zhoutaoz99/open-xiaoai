...
import { kMarkerPrefix, kMarkerSuffix } from "../memory/marker";
 ...
 * 所以两个方向都要写死。marker 是给流式工具调用不稳的服务商兜底的，
 * 语义和 tools 完全一样。
 */
...
- 但家人让你"提醒""记个待办""别让我忘了"这类，是要你到点主动提醒——**该用待办工具就用**，别当成普通记忆敷衍过去（没挂待办工具时才照上一条处理）。用待办工具时**先别开口，等记好了再一句话确认**，别调用前先说一遍、确认时又说一遍。`;
}
...
export class SoulService {
  ...
  systemPrompt(): string {
    ...
    if (this.config.systemPrompt) {
      ...
      parts.push(this.soulText(), kSpeechRules);
    }
