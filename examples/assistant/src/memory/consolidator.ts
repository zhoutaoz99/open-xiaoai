...
import { normalizeOp } from "./store.js";
import type { TranscriptEntry } from "./transcript.js";
import { today, type MemoryItem, type MemoryOp } from "./types.js";
 ...
 *
 * @returns 解析或校验失败返回 undefined，调用方应保持原状
 */
...
function buildPrompt(input: ConsolidateInput): string {
  ...
  const transcript = recent.length
    ? recent.map((e) => `[${e.ts.slice(0, 16)}] 用户：${e.user}`).join("\n")
    : "（暂无）";
