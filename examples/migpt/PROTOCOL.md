...
| `done` | `{}` 或 `{"keep_awake":false}` | 正常结束。**必须发送**，migpt 据此结束播报；`keep_awake` 见 5.4 |
| `error` | `{"message":"...","text":"..."}` | 出错。`text` 为可选的兜底播报话术 |
| `fallback` | `{}` | 放弃本次回答，交回小爱。**必须在任何 `delta` 之前发送** |

流式下 `keep_awake` 挂在 `done` 事件上，migpt 要等这句播完才收得到——这正是判定要不要进连续对话的时机。
...
开启连续对话（migpt 侧 `KEEP_AWAKE=true`）后，migpt 每播完一条**有内容**的回复，就会自动再开一个约 7 秒的收音窗口，让用户不用重说唤醒词就能接着问。`keep_awake` 是外部服务对这个行为的**一票否决**：

| 取值 | 含义 |
| --- | --- |
| 省略 / `true` | 外部服务没意见，由 migpt 的 `KEEP_AWAKE` 配置决定（默认行为） |
...
1. **只在 `KEEP_AWAKE=true` 时有意义。** 没开连续对话时本就不会自动续窗，`keep_awake: false` 是无操作。外部服务可以无脑带上，不用关心 migpt 那边开没开。
